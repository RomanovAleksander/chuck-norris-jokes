...
export const JokeItem = ({ joke, isFavourite, addJoke, removeJoke, favourites }) => {
  const { url, id, value, updated_at, categories } = joke;

  const convertDate = (date) => {
    return Math.floor((Date.now() - Date.parse(date)) / (1000 * 60 * 60));
  };

  const isFav = favourites === true ? 'joke-item white' : 'joke-item dark';

  return (
    <div className={isFav}>
      <button className="joke-item__btn" onClick={isFavourite === undefined ? addJoke : removeJoke}>
        {isFavourite ?
      ...
      <div className="joke-item__body">
        <p className="joke-item__link">
          ID:{" "}
          <a
            className="joke-item__link"
            target="_blank"
        ...
        <p className="joke-item__content">{value}</p>
      </div>
      <div className="joke-item__footer">
        <div className="joke-item__last-update">
          Last update:{" "}
          <span>
            {convertDate(updated_at)} hours ago
          </span>
        </div>
        {categories.length ? (
          <div className="joke-item__category">{categories[0]}</div>
        ) : null}
      </div>
    </div>
  )
